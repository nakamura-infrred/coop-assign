...
ボランティア的に属人化している人員割り振り作業を、**「割り振り担当者の負荷を軽減する支援ツール」**として運用できるようにする OSS プロジェクトです。カレンダー UI 上で担当状況を可視化し、ハード制約チェックや軽量フェアネス表示で担当者の判断を支える MVP を目指します。

### 基本方針

- **担当者ファースト**: 「みんなで納得感を作る」といった壮大な目的よりも、まずは「割当担当者が楽になること」を最優先します。
- **レジリエンス（継続性）重視**: 万が一システムが停止しても、CSV 出力や印刷物があればアナログ運用に戻れる（業務が停滞しない）疎結合な設計を維持します。
- **シンプル＆実用**: 複雑な自動割当よりも、直感的な操作とメンテナンスの容易さを重視します。
